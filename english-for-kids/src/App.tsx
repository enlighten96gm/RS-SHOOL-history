...
const App = () => {
  ...
  const setsArr: any = []
  SetsApi.getSets(adminName).then(res => { res.map((item: any) => setsArr.push(item))})
  // Request for every single set
  ...
  const RouterAdminWords = routesNames.map((item, id) => {
    return <Route path={`/${item[0]}category`} render={() => <WordsPanel item={item} adminName={adminName}/>} />
  })
