...
const SingleCategoryComp = ({item, adminName, rerenderHeandler, changeColor, setCorrectPath}: any) => {
    ...
    return (
        ...
        <div>
            ...
            <div onClick={deleteHeandler} className={css.delete}>DELETE</div>
            <div className={css.heading}>{item[0]}</div>
                <div>Words: {wordLength}</div>
            <div className={css.buttons}>
                <div onClick={updateHeandler} className={css.firstBtn}>Update</div>
                <NavLink onClick={changeColor} className={css.link} to={`/${item[0]}category`}>
                    <div onClick={handleCorrectPath} className={css.secondBtn}>Add Word</div>
                </NavLink>
            ...
            <div className={css.img__block}><input onChange={handleFile} className={css.img__block__file} type="file"/><div className={css.swagg}>Change IMG</div></div>
            <div className={css.buttons}>
                <div onClick={updateHeandler} className={css.firstBtn}>Cancel</div>
                <div onClick={changeFullHeandler} className={css.secondBtn}>Create</div>
            </div>
        </div>
            }
        </div>
